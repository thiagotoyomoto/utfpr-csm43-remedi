    ...
    const onChange = ({type}, selectedDate) => {
        if (type == "set") {
            ...
            if(Platform.OS === "android"){
                ...
                const dateToString = () => {
                    var str = ""  
                    currentDate.getDate() < 10 ? str+= `0${currentDate.getDate()}` : str+=`${currentDate.getDate()}`;
                    currentDate.getMonth() < 9 ? str+= `/0${currentDate.getMonth()+1}` : str+=`/${currentDate.getMonth()+1}`;
